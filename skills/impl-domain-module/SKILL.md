...
- 每个领域子包下**逻辑职责**划分如下：
  ...
  - **factory/**：工厂**接口**，**只能包含两个方法**：**create(用户创建该领域对象时可能填写的业务字段…)**、**createByNum(String num)**；两个方法都用于构建领域对象；`create(...)` 的参数只能是用户创建该领域对象时可能填写的字段，不得包含操作人、创建人/更新人、状态、审计字段、系统生成编号、默认值、流程流转字段等用户不可见且用于内部流转的内容；`createByNum` **必须**等价于委托 `repository.findByNum(num)` 按业务编码加载并构建既有领域对象；**禁止**定义 `createById(...)`、`rebuild(...)` 或其他任何工厂方法；返回 domain 实体；实现类在 infra。
...
| **factory/** | 工厂**接口**；只能包含两个方法：`create(用户创建该领域对象时可能填写的业务字段…)`、`createByNum(String num)`；两个方法都用于构建领域对象；`create(...)` 不包含操作人、状态、审计字段、系统生成编号等内部流转字段；`createByNum` 按业务编码委托 `repository.findByNum(num)`；禁止定义其他工厂方法；实现类在 infra。 |
...
2. **领域工厂接口**只能包含两个方法：**create**、**createByNum**；两个方法都用于构建领域对象；`create(...)` 根据用户创建该领域对象时可能填写的业务字段构建新的领域对象，参数不得包含操作人、创建人/更新人、状态、审计字段、系统生成编号、默认值、流程流转字段等用户不可见且用于内部流转的内容；**createByNum(String num)** 的实现**只能**转调对应仓储的 **`findByNum(num)`**，根据业务编码加载并构建既有领域对象，不得在工厂内绕过仓储做持久化；**禁止**定义 `createById(...)`、`rebuild(...)` 或其他任何工厂方法。
