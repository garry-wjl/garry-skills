...
- **依赖**：**仅依赖 client、domain、infra**。CommandService 可注入 domain 的 Factory 与其他领域 QueryService；QueryService 可注入 infra Mapper；application 层**禁止注入或调用任何 domain Repository 接口**，**禁止注入或调用任何领域 Gateway 接口**，Repository 与 Gateway 仅供领域对象、领域工厂及 infra 实现内部使用；不直接依赖 adapter 或 facade（除 facade 中通用类型若被 domain 使用则经 domain 间接依赖）。
...
- **MyBatis Plus Wrapper 约束**：QueryService 通过 Mapper 做只读查询时**必须使用 `LambdaQueryWrapper<Entity>`**，通过实体字段的 lambda 引用（如 `.eq(Entity::getField, value)`）构造条件；禁止使用 `QueryWrapper` 的字符串列名重载。
...
5. **Repository 与 Gateway 禁用约束**：application 层任何 Service（CommandService / QueryService / StreamService）都不得注入或调用 domain Repository 接口或领域 Gateway 接口；Repository 与 Gateway 是领域层内部持久化与协作接口，仅供领域对象、Factory 及 infra 实现使用。
6. **写操作必须使用 Redis 分布式锁**：CommandService 中涉及写操作的 CommandService 方法（如创建、更新、删除、提交流程等），必须使用 **Redis 分布式锁**（如 `RLock`、`StringRedisTemplate.setIfAbsent`、或自定义注解 `@RedisLock`），以应对多实例部署下的并发重复请求。锁 key 须基于业务唯一标识（如 `num`、订单号、会话编码），不能使用固定值；锁的粒度精确到被操作的业务对象；必须设置合理的过期时间防止死锁。
7. 需要事务时在 application 方法上加 `@Transactional(rollbackFor = Exception.class)`，不在 domain 或 infra 单方法上随意加事务；事务范围应内嵌在分布式锁区域内（先获取锁，再开启事务）。
8. 不注入 adapter 或 HTTP 相关类；操作人 ID 等由 adapter 传入 application 方法参数。
