...
3. **领域模型设计（必选）**  
   遵循**先按业务分层级，再按业务领域分节、每领域下依次设计领域模型、领域工厂、领域规则、领域动作、领域事件**的原则。须按 DDD 规范设计，便于与 **impl-domain-module** 对齐落码，并满足**领域模型基本属性与必备动作**约定。  
   **文档目录结构**：  
   ...
   - 示例：`3.2 用户（user）` → `3.2.1 领域模型`、`3.2.2 领域工厂`、`3.2.3 领域规则`、`3.2.4 领域动作`、`3.2.5 领域事件`；`3.3 家庭（family）` → `3.3.1 领域模型`、`3.3.2 领域工厂`、`3.3.3 领域规则`、`3.3.4 领域动作`、`3.3.5 领域事件`；依此类推。
   ...
   **各三级目录内容要求**（在每一业务领域下分别填写）：
   ...
   - **领域工厂**（如 3.x.2）  
     ...
     - **方法清单**：使用表格列出工厂方法，列：Factory、方法名、入参、返回值、职责、依赖（Repository/Gateway/Mapper 等）。一般情况下，领域工厂包含两个核心方法：`create(...)`（根据属性创建新的领域对象）、`createByNum(...)`（根据业务编码 num 构建既有领域对象，方法内部通过 Repository 获取并重建领域对象）。如确有需要，可补充 `createById(...)`、`rebuild(...)` 等辅助方法，但不作为默认必选方法。  
     - **时序说明**：`createByNum(...)` 的时序须体现通过 Repository/仓储获取数据并构建领域对象；涉及复杂构建、跨仓储加载或外部依赖时，须补充工厂构建时序图。
...
- [ ] **领域模型设计**按**业务领域为二级目录**（如 3.2 用户、3.3 家庭、3.4 账本…），每个业务领域下包含**五个三级目录**：**领域模型**、**领域工厂**、**领域规则**、**领域动作**、**领域事件**；各节内容符合上条（类图+表格、工厂方法表、规则表、动作表+每动作一时序图、事件表）；与模块变更清单中 domain 层描述一致。
