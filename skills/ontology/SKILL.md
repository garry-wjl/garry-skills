...
```bash
python3 .cursor/skills/ontology/scripts/ontology.py create --type Person --props '{"name":"Alice","email":"alice@example.com"}'
```
...
```bash
python3 .cursor/skills/ontology/scripts/ontology.py relate --from proj_001 --rel has_task --to task_001
```

### Validate

```bash
python3 .cursor/skills/ontology/scripts/ontology.py validate  # Check all constraints
```

（以上命令均在**项目根目录**下执行；数据文件为项目根下 `memory/ontology/graph.jsonl`。）
...
```bash
# Initialize ontology storage (from project root)
...
# Start using (from project root)
python3 .cursor/skills/ontology/scripts/ontology.py create --type Person --props '{"name":"Alice"}'
