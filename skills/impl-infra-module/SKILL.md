...
- 每个领域子包下**仅包含**以下目录：
  ...
  - **factory**：实现 domain 的 Factory 接口；**构造器/字段仅注入本聚合的 Repository**，**不得**注入 Gateway、DomainEventPublisher；一般情况下包含 `create(...)` 与 `createByNum(String num)` 两个核心方法，其中 `create(...)` 根据属性构建新的领域对象，`createByNum(String num)` 必须委托 `Repository.findByNum(num)` 按业务编码加载并构建既有领域对象。若领域聚合根仍依赖 Gateway/Publisher：**由应用层**在工厂/仓储返回后补全依赖，或**先重构领域模型/构造方式**使加载路径不依赖在 infra 仓储内持有上述 Bean；**禁止**指望 RepositoryImpl 代为注入或组装。
...
| **factory** | 仅注入本聚合 Repository；不注入 Gateway、DomainEventPublisher；默认实现 `create(...)` 与 `createByNum(String num)`，后者仅委托 `Repository.findByNum(num)`。 |
...
7. **FactoryImpl**：**仅**注入本聚合 **Repository**；**禁止**注入 **Gateway**、**DomainEventPublisher**。一般情况下包含两个核心方法：`create(...)` 根据属性构建新的领域对象；`createByNum(String num)` 仅委托 `Repository.findByNum(num)`，不得绕过仓储或调用其他查询。
