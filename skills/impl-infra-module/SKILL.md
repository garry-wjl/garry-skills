...
- 每个领域子包下**仅包含**以下目录：
  ...
  - **factory**：实现 domain 的 Factory 接口；**构造器/字段仅注入本聚合的 Repository**，**不得**注入 Gateway、DomainEventPublisher；FactoryImpl **只能实现** `create(...)` 与 `createByNum(String num)` 两个方法，两个方法都用于构建领域对象；`create(...)` 根据属性构建新的领域对象，`createByNum(String num)` 必须委托 `Repository.findByNum(num)` 按业务编码加载并构建既有领域对象；**禁止**实现 `createById(...)`、`rebuild(...)` 或其他任何工厂方法。若领域聚合根仍依赖 Gateway/Publisher：**由应用层**在 Factory 返回后补全依赖，或**先重构领域模型/构造方式**使加载路径不依赖在 infra 仓储内持有上述 Bean；**禁止**指望 RepositoryImpl 代为注入或组装。
...
7. **FactoryImpl**：**仅**注入本聚合 **Repository**；**禁止**注入 **Gateway**、**DomainEventPublisher**。只能实现两个方法：`create(...)` 根据属性构建新的领域对象；`createByNum(String num)` 仅委托 `Repository.findByNum(num)` 构建既有领域对象，不得绕过仓储或调用其他查询；禁止实现 `createById(...)`、`rebuild(...)` 或其他任何工厂方法。
