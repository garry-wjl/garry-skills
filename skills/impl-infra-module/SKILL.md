...
- 每个领域子包下**仅包含**以下目录：
  - **entity**：与表结构对应的 PO，MyBatis Plus 注解；含审计字段与 is_deleted；**is_deleted 仅属于 infra Entity/DB 持久化字段，不得映射到聚合根或领域实体**；**必须使用 Lombok `@Data`**，**禁止手写 getter/setter**；**每个字段须有注释**（类内行注释或字段 Javadoc），说明含义、单位或约束。
  - **mapper**：MyBatis Plus 的 BaseMapper<Entity> 及自定义 SQL（XML/注解）。
  - **repository**：实现 domain 的 Repository 接口；Entity↔领域实体转换；**构造器与字段仅允许本聚合表对应的 `BaseMapper<?>` 实现类**（含本聚合子表 Mapper）；**绝对禁止**注入 **任何形式的** `DomainEventPublisher`、`ApplicationEventPublisher`、**任何**领域 **Gateway**、其他聚合 Mapper、其他 Repository、Application Service 等——**除 Mapper 外不得出现任何可注入依赖**；判断「是否存在」时**必须用业务编码 `num` 查库**，**查到即存在，查不到即不存在**；RepositoryImpl 仅作为领域对象/Factory 的持久化协作实现，不作为 application 查询入口；**事务与分布式锁**：在 **application** 层声明（如 `@Transactional`），**禁止**在 RepositoryImpl 注入 `PlatformTransactionManager`、`TransactionTemplate`、Lock 客户端等非 Mapper Bean。
  - **factory**：实现 domain 的 Factory 接口；**构造器/字段仅注入本聚合的 Repository**，**不得**注入 Gateway、DomainEventPublisher；FactoryImpl **只能实现** `create(...)` 与 `createByNum(String num)` 两个方法，两个方法都用于构建领域对象；`create(...)` 根据用户创建该领域对象时可能填写的业务字段构建新的领域对象，参数不得包含操作人、状态、审计字段、系统生成编号等内部流转字段；`createByNum(String num)` 必须委托 `Repository.findByNum(num)` 按业务编码加载并构建既有领域对象；**禁止**实现 `createById(...)`、`rebuild(...)` 或其他任何工厂方法。若领域聚合根仍依赖 Gateway/Publisher：**由应用层**在 Factory 返回后补全依赖，或**先重构领域模型/构造方式**使加载路径不依赖在 infra 仓储内持有上述 Bean；**禁止**指望 RepositoryImpl 代为注入或组装。
...
| 目录 | 职能 |
|------|------|
| **entity** | 表映射 PO；`@Data`；字段全注释；@TableName/@TableId/@TableField；审计字段 + is_deleted；is_deleted 仅限 DB/Entity，不映射到领域模型；无业务逻辑。 |
...
| **factory** | 仅注入本聚合 Repository；不注入 Gateway、DomainEventPublisher；只能实现 `create(...)` 与 `createByNum(String num)` 两个方法，后者仅委托 `Repository.findByNum(num)`；禁止其他工厂方法。 |
...
- **MyBatis Plus Wrapper 约束**：操作数据库时**查询必须使用 `LambdaQueryWrapper<Entity>`**，**更新必须使用 `LambdaUpdateWrapper<Entity>`**，通过实体字段的 lambda 引用（如 `.eq(Entity::getField, value)`）构造条件；禁止使用 `QueryWrapper`、`UpdateWrapper` 的字符串列名重载或手动 `Map` 参数拼接条件。
...
4. 领域实体与 Entity 的转换仅在 infra 的 Repository/Factory 内进行，不把 Entity 暴露给 application 或 domain；application 查询场景通过对应领域 QueryService 注入 Mapper 完成，不通过 Repository。
...
7. **FactoryImpl**：**仅**注入本聚合 **Repository**；**禁止**注入 **Gateway**、**DomainEventPublisher**。只能实现两个方法：`create(...)` 根据用户创建该领域对象时可能填写的业务字段构建新的领域对象，参数不得包含操作人、状态、审计字段、系统生成编号等内部流转字段；`createByNum(String num)` 仅委托 `Repository.findByNum(num)` 构建既有领域对象，不得绕过仓储或调用其他查询；禁止实现 `createById(...)`、`rebuild(...)` 或其他任何工厂方法。
