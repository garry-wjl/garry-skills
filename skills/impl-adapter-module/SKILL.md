...
| 目录 | 职能 |
|------|------|
| **controller/** | 该领域的 HTTP 控制器；*CommandController（写、POST）、*QueryController（读、GET）；或单一 *Controller。入参必须使用 `*Param` 类（定义在 client 层）并配合 `@Valid` 校验；出参（除 `Result` 等全局包装结构外的业务对象）必须使用 `*VO` 类（定义在 client 层）。若应用层返回的是 `DTO` 类型（非 `VO`），控制器须负责 `DTO` 到 `VO` 的转换。只调 application 一层；返回 `Result.ok(data)` 或 `Result.fail(msg)`。 |
...
1. **入参必须是 `*Param` 类**：Controller 方法的请求参数（请求体参数、查询参数）必须使用 `*Param` 类封装，定义在 client 层，并配合 `@Valid` 进行参数校验。禁止使用 `DTO`、领域对象或零散原始类型作为 Controller 入参。
2. **出参必须是 `*VO` 类**：Controller 返回的业务数据（除 `Result` 等全局包装结构外）必须使用 `*VO` 类封装，定义在 client 层。若应用层返回的是 `DTO` 类型（与 `VO` 字段或结构不一致），控制器须在调用应用层后负责将 `DTO` 转换为 `VO` 再返回。
3. 每个接口方法：参数校验（`*Param` + `@Valid`）→ 类型转换（如需要，`DTO` → `VO`）→ 调 application 一层方法 → 返回 `Result.ok(vo)` 或 `Result.fail(msg)`。
4. 不在 controller 或事件消费者中写 if/else 业务分支；复杂分支放在 application 或 domain。
5. 新 Controller 继承 **BaseController**（若项目有），通过 `getCurrentUserId()` 获取操作人并传入 application；否则通过 request 或 application 提供的上下文获取操作人。
6. 接口约定：**GET** 表示查询，**POST** 表示写操作（与项目 API 约定一致）。
